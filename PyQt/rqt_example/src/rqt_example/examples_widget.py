...
class ExamplesWidget(QWidget):
    ...
    def __init__(self, node):
        ...
        current_position_name = 'turtle1/pose'
        transition_name = 'turtle1/teleport_absolute'
        ...
        self.subscriber = self.node.create_subscription(Pose,current_position_name,self.get_position,qos)
        self.cli = self.node.create_client(TeleportAbsolute,transition_name)
        ...
        self.rbt_27.clicked.connect(self.call_transition_service)
        self.rbt_38.clicked.connect(self.call_transition_service)
        self.rbt_55.clicked.connect(self.call_transition_service)
        self.rbt_72.clicked.connect(self.call_transition_service)
    ...
    def set_led_status(self, request, response):
        if request.data:
            ...
            self.push_button_led_status.setStyleSheet('color: rgb(0,0 , 255);')
            response.success = True
    ...
    def get_position(self,msg):
        self.lcd_xp.display(msg.x)
        self.lcd_yp.display(msg.y)
    
    def call_transition_service(self):
        req = TeleportAbsolute.Request()
        if self.rbt_27.isChecked():
            req.x = 2.0
            req.y = 7.0
        elif self.rbt_38.isChecked():
            req.x = 3.0
            req.y = 8.0
        elif self.rbt_55.isChecked():
            req.x = 5.0
            req.y = 5.0
        elif self.rbt_72.isChecked():
            req.x = 7.0
            req.y = 2.0
        ...
        future.add_done_callback(
            lambda f: self.node.get_logger().info("Teleport success")
            if f.result() is not None
            else self.node.get_logger().error("Service call failed")
        )
